...
export function searchSuppliers(query: string): Supplier[] {
  ...
  // Match suppliers by name/location/description
  suppliers.forEach((s) => {
    if (
  ...
  // Also match suppliers who have matching products
  products.forEach((p) => {
    if (
      p.name.toLowerCase().includes(q) ||
      p.category.toLowerCase().includes(q)
