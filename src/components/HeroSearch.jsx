...
const HeroSearch = () => {
  ...
  const quickSearches = useMemo(
    () => ["Cement", "TMT Steel", "Tiles", "Paints", "Plywood", "Electricals", "Hardware"],
    []
  );
  ...
  const handleSearch = useCallback(
    (e) => {
      ...
      if (!trimmed || trimmed === currentQuery) return;

      navigate(`/search?q=${encodeURIComponent(trimmed)}`);
    },
    [query, navigate, currentQuery]
  );
    ...
    <section className="py-12 md:py-20 text-center animate-fade-in">
      ...
      <form onSubmit={handleSearch} className="max-w-xl mx-auto flex gap-2">
        ...
        <Button
          ...
          disabled={!query.trim() || query.trim() === currentQuery}
        >
          Search
        </Button>
      </form>
