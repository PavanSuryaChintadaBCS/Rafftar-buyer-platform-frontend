        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl font-bold flex items-center gap-2">
              <MessageSquare className="h-5 w-5" /> {ticket.id}
            </h1>
          ...
          <CardContent className="p-4">
            <div className="space-y-3 max-h-96 overflow-y-auto mb-4">
              {ticket.messages.map((msg) => (
                <div key={msg.id} className={cn("flex", msg.sender === "buyer" ? "justify-end" : "justify-start")}>
                  <div className={cn(
                    "max-w-[75%] rounded-xl px-4 py-2",
                    msg.sender === "buyer" ? "bg-primary text-primary-foreground" : "bg-secondary"
                  )}>
                    <p className="text-sm">{msg.text}</p>
                    <p className={cn("text-xs mt-1", msg.sender === "buyer" ? "text-primary-foreground/70" : "text-muted-foreground")}>
