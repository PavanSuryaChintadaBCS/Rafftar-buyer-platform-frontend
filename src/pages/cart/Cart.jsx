...
const Cart = () => {
  const { items, removeItem, updateQuantity, clearCart, grouped } = useCart();
  const { buyer, ready } = useAuthGuard({ from: "/cart" });
  const navigate = useNavigate();

  if (!ready) return null;

  const { subtotal, logistics, tax: gst, total } = calcOrderTotals(
    items,
    getProductById,
    buyer.type
  );
                          ...
                          >
                            ...
                            <div className="text-right flex-shrink-0">
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  removeItem(item.productId, item.supplierId)
                                }
