      ...
      <main className="container mx-auto px-4 py-6 max-w-2xl">
        ...
        {items.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-sm text-muted-foreground mt-1">Browse products and add them to your inquiry.</p>
          </div>
        ) : (
              ...
              <Card key={item.product.id}>
                <CardContent className="p-4 flex gap-4 items-center">
                  <div className="h-16 w-16 bg-secondary/30 rounded-lg flex items-center justify-center flex-shrink-0">
                    <img src={item.product.image} alt={item.product.name} className="h-8 w-8 opacity-40" />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-bold mt-1">₹{item.product.price}/{item.product.unit}</p>
                  </div>
