...
type KYCStep = "business" | "documents" | "verification";

const KYC = () => {
  const { buyer, toggleKYC } = useBuyer();
  const navigate = useNavigate();
  const [step, setStep] = useState<KYCStep>("business");
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-4 py-16 max-w-md text-center">
          ...
          <p className="text-muted-foreground mb-6">Your business is verified. You have full access to all features.</p>
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 max-w-lg">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {[
                ...
                <div key={label} className={cn("border rounded-lg p-4 flex items-center justify-between", uploaded && "border-primary bg-primary/5")}>
                  <div className="flex items-center gap-3">
                    {uploaded ? <CheckCircle2 className="h-5 w-5 text-primary" /> : <Upload className="h-5 w-5 text-muted-foreground" />}
                    <div>
                      <p className="text-sm font-medium">{label}</p>
                      <p className="text-xs text-muted-foreground">{uploaded ? "Uploaded" : "PDF, JPG, PNG (max 5MB)"}</p>
                    </div>
                  </div>
                  {!uploaded && (
                    <Button variant="outline" size="sm" onClick={() => simulateUpload(setter)}>
                      Upload
                    </Button>
                  )}
                </div>
        ...
        {step === "verification" && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Review & Verify</CardTitle>
            ...
            <CardContent className="space-y-4">
              <div className="bg-secondary/50 rounded-lg p-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {[panUploaded, gstUploaded, addressProofUploaded].filter(Boolean).length}/3 uploaded
                  </span>
                </div>
              </div>
              ...
              <div className="border rounded-lg p-4 text-sm">
                ...
                <ul className="text-muted-foreground space-y-1 text-xs">
                  <li>• Your documents will be verified (simulated instantly)</li>
                  <li>• Full pricing and bulk discounts will be unlocked</li>
              ...
              <div className="flex gap-3">
                ...
                <Button onClick={handleVerify} className="flex-1 gap-2" disabled={loading}>
                  {loading ? (
                    ...
                    <><Shield className="h-4 w-4" /> Complete KYC</>
                  )}
                </Button>
              </div>
            </CardContent>
